...
fftList = ["FFT","RFFT","IRFFT","DCT","DST"]
fft = StringVar()
fft.set(fftList[0])
ffDrop = OptionMenu(root, fft, *fftList)
ffDrop.grid(column=0, row=4, sticky=W, **paddings)
#######################
calcButton = Button(root, text="Visualize", command=plot)
calcButton.grid(column=1,row=3, sticky=W, **paddings) 
trfButton = Button(root, text="Transform", command=plot2)
trfButton.grid(column=1,row=4, sticky=W, **paddings) 
